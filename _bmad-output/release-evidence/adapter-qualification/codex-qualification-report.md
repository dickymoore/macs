...
- Runtime binary or backend tested: codex
...
- Known unsupported features: token_budget, structured_progress, checkpoint_hints
...
| Permission surface | Approval/sandbox signals preserved where available | optional enrichments: runtime_permission_surface | PASS |
...
- Unsupported intervention behaviors: token_budget, structured_progress, checkpoint_hints
...
| --- | --- | --- | --- | --- |
| degraded_mode_behavior | Falls back to tmux-observed pane presence and declared capabilities when Codex CLI flags are unavailable. | n/a | required | shared contract declaration |
| unsupported_features | token_budget, structured_progress, checkpoint_hints | n/a | optional | explicit unsupported declaration |
...
- Example inspector output: `macs adapter inspect --adapter codex --json`
...
| --- | --- | --- | --- |
| Declared status is reference while release eligibility is evidence-based. | medium | rely on the release-gate evidence rather than declaration alone | eng |
